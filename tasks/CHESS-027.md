...
- **팔레트 — 결정됨 (2026-08-18): 지금 토큰을 그대로 둔다.** 레퍼런스는 채도 높은 오렌지
  오크(색상 32도, 채도 0.78)이고 지금 토큰은 크림/토프(`--light` 채도 0.19)다.
  Divine Eye 의 `hueZoneParity 0.0` 은 이 차이를 재는 것이고, 앞으로도 그럴 것이다 —
  `low-contrast-field` 0.62 는 **미달이 아니라 선언한 편차의 측정값**으로 남는다.
  레퍼런스는 이 판에 구조와 관계만 주고 색은 주지 않는다.
- 두께가 위에서 안 읽히는 문제를 시점으로 풀지, 두께로 풀지. (열려 있음)
